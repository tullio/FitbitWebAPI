...
class DataSeries(startDateTimeObj: DateTime):
    val xData = scala.collection.mutable.ArrayBuffer.empty[Double]
    val yData = scala.collection.mutable.ArrayBuffer.empty[Double]
    val sleepXData = scala.collection.mutable.ArrayBuffer.empty[Double]
    val sleepYData = scala.collection.mutable.ArrayBuffer.empty[Double]
    val activeXData = scala.collection.mutable.ArrayBuffer.empty[Double]
    val activeYData = scala.collection.mutable.ArrayBuffer.empty[Double]
    val avgXData = scala.collection.mutable.ArrayBuffer.empty[Double]
    ...
    def setHeartRateData(dataSeries: List[Either[String, Dataset]]) =
        ...
        if generatePNG then
            fig.addSignal("Heart Rate", xData.toArray, yData.toArray, false)
    ...
    def setSleepData(sleepData: Sleep) =
        ...
        if sleepData.sleep.length > 0 then
            ...
            if generatePNG then
                fig.addSignal("Sleep", sleepXData.toArray, sleepYData.toArray, false)
    def setActiveData(activeData: ActivityLog) =
        ...
        if generatePNG then
            fig.addSignal("Active", activeXData.toArray, activeYData.toArray, false)
            fig.addSignal("Active(avg)", avgXData.toArray, avgYData.toArray, false)
